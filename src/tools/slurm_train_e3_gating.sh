#!/bin/bash
#SBATCH --job-name="centerpoint_e3_gating"
...
#SBATCH --output=outputs/slurm_centerpoint_e3_gating_%j.out
#SBATCH --error=outputs/slurm_centerpoint_e3_gating_%j.err
...
default_args=(model=centerpoint_radar_pfn2_gate exp_id=radar_pfn2_gate batch_size=4 num_workers=2 epochs=30)
train_args=("${default_args[@]}" "$@")

printf 'Launching E3 gating training with args:\n'
printf '  %s\n' "${train_args[@]}"
